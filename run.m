...
imageOriginalPath = 'C:\Users\Jay\Desktop\Research\CS_based_image_compression';
...
simulation_parameter.bit                             = [128 64 32 16 8 4];
            ...
            for frame_number = 1:1
                ...
                for k = 1:plane
                    for i = 1:size(frame,1)/simulation_parameter.macro_block_size
                        for j = 1:size(frame,2)/simulation_parameter.macro_block_size
                            %___SQ coding___
                            bit_depth                                       = log2(simulation_parameter.bit(quantization_bit)); % compute number of levles    
                            quantization.quantization_parameter_SQ(i,j,k)   = (max(y.measurement{i,j,k})-min(y.measurement{i,j,k}))/2^bit_depth; % compute step size of uniform quantizer
                            y.quantized_SQ{i,j,k}                           = round(y.measurement{i,j,k}/quantization.quantization_parameter_SQ(i,j,k));  %compute quantized image
                            y.quantized_SQ{i,j,k}(isinf(y.quantized{i,j,k})|isnan(y.quantized_SQ{i,j,k})) = 0; % Replace NaNs and infinite values with zeros
                ...
                quantitative_metrices.psnr_SQ(quantization_bit)             = PSNR(uint8(cell2mat(x_hat_SQ(:,:,:,frame_number))), frame);
                quantitative_metrices.ssim_SQ(quantization_bit)             = ssim(uint8(cell2mat(x_hat_SQ(:,:,:,frame_number))), frame);
                quantitative_metrices.entropy_bits_SQ(quantization_bit)     = residual_entropy_SQ;
            end
    end
end

figure;
plot(quantitative_metrices.entropy_bits_SQ,quantitative_metrices.ssim_SQ, 'r-x', 'LineWidth', 2);
hold on
title('Barbara');
xlabel('Bitrate (bpp)');
ylabel('SSIM');

% legend('This work', 'SQ', '[1]', '[2]','Location', 'SouthEast');
% temp = uint8(cell2mat(reconstructed_image(:,:,:,frame_number)));
% imwrite(temp(100:200,100:200),'cropped32.bmp');
% imwrite(temp,'32.bmp');
